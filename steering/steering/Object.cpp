...
Object& Object::SetPower(std::array<double,4>length, std::array<double,4>angle) {
	...
	for (int i = 0; i < 4; i++) {
		...
		Shape2D::Arrow(Rectframe[i].center().rotatedAt(Robot.center(), Robot_Vec.z)
			, { Rectframe[i].center().rotatedAt(Robot.center(), Robot_Vec.z).x + Vector[i].x
			, Rectframe[i].center().rotatedAt(Robot.center(), Robot_Vec.z).y - Vector[i].y }
			, 6
			, Vec2{ 8,8 }).draw();
		...
		old_angle[i] = angle[i];
		/*回転ベクトル以外のベクトルを求める*/
		Robot_Vec.x += Vector[i].rotatedAt(Robot.center(), Robot_Vec.z).x/100;
		Robot_Vec.y += Vector[i].rotatedAt(Robot.center(), Robot_Vec.z).y/100;
		/*出来てない*/
		Robot_Vec.z += (/*ｘ軸*/length[i] * sin(ToRadians(angle[i]) + (i * -0.5 - 0.75) * Math::Pi) - /*ｙ軸*/length[i] * cos(ToRadians(angle[i]) + (i * -0.5 - 0.25) * Math::Pi))/1000;
		//Robot_Vec.z += Vec2{ Vector[i].x,Vector[i].y }.rotatedAt(Robot.center(), Robot_Vec.z).rotate((i * -0.5 + 0.25) * Math::Pi).x / 10000;
		Print << U"::";
		Print << length[i] * sin(ToRadians(angle[i]) + (i * -0.5 - 0.75) * Math::Pi);
		Print << length[i] * cos(ToRadians(angle[i]) + (i * -0.5 - 0.25) * Math::Pi);
	}
